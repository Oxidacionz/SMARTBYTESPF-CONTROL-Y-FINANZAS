...
interface DebtSettlementModalProps {
  ...
  onConfirm: (amount: number, method: 'money' | 'asset_out' | 'asset_in', details: any) => void;
  onClose: () => void;
}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-200 dark:border-gray-700 flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
              <button type="button" onClick={() => setMethod('money')} className={`p-3 rounded-lg border-2 flex flex-col items-center gap-2 transition-all ${method === 'money' ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300' : 'border-gray-200 dark:border-gray-700'}`}>
                  <Coins size={24} /> <span className="text-sm font-bold">Dinero</span>
              </button>
              <button type="button" onClick={() => setMethod('asset')} className={`p-3 rounded-lg border-2 flex flex-col items-center gap-2 transition-all ${method === 'asset' ? 'border-purple-500 bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-300' : 'border-gray-200 dark:border-gray-700'}`}>
