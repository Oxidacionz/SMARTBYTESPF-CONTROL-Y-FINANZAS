      ...
      <MainLayout
        ...
        navigation={
          <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 mb-2">
            <div className="flex space-x-2 overflow-x-auto pb-2 scrollbar-hide w-full xl:w-auto">
              {[
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-hide w-full xl:w-auto justify-start xl:justify-end">
              <Button variant="secondary" size="sm" onClick={() => setShowShoppingModal(true)} icon={<ShoppingBag size={14} />} className="whitespace-nowrap bg-white dark:bg-gray-800 border border-orange-200 text-orange-600 hover:bg-orange-50 text-xs h-8 px-2">Gastos Hormiga</Button>
              <Button variant="secondary" size="sm" onClick={() => setShowDistributionChart(!showDistributionChart)} icon={showDistributionChart ? <ChevronUp size={14} /> : <PieChart size={14} />} className="whitespace-nowrap bg-white dark:bg-gray-800 border border-gray-200 text-gray-600 text-xs h-8 px-2">Distribución</Button>

              <Button size="sm" onClick={() => { setEditingItem(null); setShowAddModal(true); }} icon={<Plus size={16} />} className="bg-blue-600 text-white shadow-md hover:bg-blue-700 px-3 whitespace-nowrap h-8 text-xs">+ Agregar</Button>

              <div className="relative ml-2 border-l pl-2 border-gray-200 dark:border-gray-700">
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => setShowReportsMenu(!showReportsMenu)}
                  icon={<Download size={14} />}
                  className="text-xs text-slate-400 hover:text-amber-400 whitespace-nowrap h-8 px-2"
                >
                  Reportes
                </Button>
                ...
                {showReportsMenu && (
                  <>
                    ...
                    <div className="fixed top-auto mt-1 bg-gradient-to-br from-slate-800 to-slate-900 border-2 border-slate-600/50 rounded-lg shadow-2xl z-50 min-w-[180px]">
                      <button
